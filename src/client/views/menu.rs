...
pub fn view_rules_menu<'a>(state: &'a App) -> Element<'a, AppMessage> {
    let rules_body = column![
        ...
        text("Jede Runde in Wizard hat denselben Ablauf:").size(20),
        text("Am Anfang jeder Runde wird ein Dealer und der Trumpf bestimmt"),
        text("Wenn der Trumpf ein Wizard ist, wird die Trumpf-Farbe vom Dealer bestimmt"),
        text("Ist der Trumpf ein Narr, gibt es in dieser Runde keinen Trumpf"),
        text("Danach setzt jeder Spieler seinen Bid, also die Anzahl der Tricks, die er in dieser Runde gewinnen muss"),
        text("Es werden so viele Karten ausgeteilt, wie die aktuelle Rundennummer angibt, also in Runde 1 eine Karte, in Runde 2 zwei Karten, etc."),
        text("ACHTUNG! - Die Gesamtzahl aller angesagten Stiche darf nicht gleich der Anzahl der möglichen Stiche sein."),
        ...
        text("Sind alle Karten ausgespielt, werden Stiche mit den Ansagen abgeglichen und die entsprechenden Punkte verteilt."),
        text("\n"),
        text("Punkte:").size(20),
        text("Hat ein Spieler genau die Anzahl an Stichen gewonnen, die er angesagt hat, bekommt er 20 Punkte plus 10 Punkte pro gewonnenem Stich."),
        text("Beispiel: 3 Stiche angesagt, 3 Stiche gewonnen -> 20 + 10*3 = 50 Punkte"),
        text("Hat ein Spieler nicht die Anzahl an Stichen gewonnen, die er angesagt hat, bekommt er -10 Punkte pro zu viel oder zu wenig gewonnenem Stich."),
        text("Beispiel: 3 Stiche angesagt, 5 Stiche gewonnen -> -10 * (5-3) = -20 Punkte"),
        text("\n"),
        text("Ende:").size(20),
        text("In der letzten Runde gibt es keinen Trumpf"),
        text("Es werden so viele Runden gespielt bis alle Karten ausgeteilt wurden, also bei 3 Spielern 20 Runden, bei 4 Spielern 15 Runden, etc."),
        text("Der Spieler mit den meisten Punkten am Ende des Spiels gewinnt!")
    ];
